...
from backend.services import generate_ghs_code
...
async def handle_update(client: httpx.AsyncClient, token: str, update: dict[str, Any]) -> None:
    ...
    if callback := update.get("callback_query"):
        ...
        if data == GENERATE_CODES_CB:
            ...
            total_codes = len(store.generated_codes)
            latest_codes = list(store.generated_codes)[-5:]
            code_preview = ", ".join(latest_codes) if latest_codes else "No codes yet"
            await send_main_menu(
                client,
                token,
                chat_id,
                f"Total generated codes: {total_codes}\nLatest: {code_preview}",
            )
        else:
